...
module Bugzilla
  # Implements a hooks system. A hook is a callable that is associated
  # with an event. Currently supported hooks are:
  #  `:after_session` `:before_session` `:when_started`
  #
  # A hook must have a name, and is connected with an event by the `Bugzilla::Hooks#add_hook` method.
  #
  # @example Adding a hook for the `:before_session` event.
  #   Bugzilla.config.hooks.add_hook(:before_session, :say_hi) do
  #     puts "hello"
  #   end
  ...
  class Hooks
    def self.default
      hooks = new
      hooks.add_hook(:before_session, :default) do |_out, _target, bugzilla_instance|
        # Todo
      end
      hooks
    end
    ...
    #   Bugzilla::Hooks.new.merge(hooks)
    # @param [Bugzilla::Hooks] other The `Pry::Hooks` instance to merge
    # @return [Bugzilla::Hooks] a new `Pry::Hooks` instance containing a merge of the
    #   contents of two `Bugzilla:Hooks` instances.
    def merge(other)
      dup.tap do |v|
        v.merge!(other)
      end
